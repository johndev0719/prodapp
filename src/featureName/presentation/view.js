const { createAProduct, showProductsThatHasStocks, updateAProduct, productIdToUpdateQuestion, productIdQuestion, rl, productIdToRemoveQuestion, removeAProduct } = require('../data/repository_imp/repositoryImp');
        ...
        rl.question('Enter product name:', name => {
            rl.question('Enter product quantity:', quantity => {
                rl.question('Enter product unit price:', async (price) => {
                    const result = await createAProduct(productModel(id, name, quantity, price));
                    console.log(result);
                    userPrompt();
                })
            })
    ...
    productIdToUpdateQuestion(id => {
        ...
        rl.question('Enter quantity new value:', quantity => {
            rl.question('Enter unit price new value:', async unit_price => {
                const results = await updateAProduct(id, {quantity: parseInt(quantity), unit_price:parseFloat(unit_price)});
